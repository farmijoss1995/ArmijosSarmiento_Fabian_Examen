package ec.ups.edu.ArmijosSarmiento_Fabian_Examen.test;
...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        File f = new File("./target/ArmijosSarmiento_Fabian_Examen.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "ArmijosSarmiento_Fabian_Examen.war").importFrom(f).as(WebArchive.class);
        
        // Add the package containing the test classes:
        war.addPackage("ec.ups.edu.ArmijosSarmiento_Fabian_Examen.test");
